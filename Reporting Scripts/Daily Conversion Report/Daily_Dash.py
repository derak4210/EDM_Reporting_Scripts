...
account_flags = accounts.copy()

conversions = download_conversion_report(str(sys.argv[1]))

#upload to database

upload_conversions(str(sys.argv[1]),conversions)
...
try:
    ...
    query = """ select date,adv as "Advertiser",
                ...
                where cd.date>='""" + str(sys.argv[1]) + """' and type='Last_Click'
...
try:
    summary_query = """
                    ...
                    where date = '""" + str(sys.argv[1]) + """' and type = 'Last_Click'
...
try:
    if (summary and data):
        subject = "Daily Conversion Report - " + str(sys.argv[1])
        ...
        try:
            file_name = "/home/ec2-user/Reports/Daily Conversion Report/Daily_Conversion_Report_" + str(sys.argv[1]) + ".csv"
        ...
        attachment.add_header('Content-Disposition', 'attachment', filename = "Daily_Conversion_Report_%s.csv" %str(sys.argv[1]) )
