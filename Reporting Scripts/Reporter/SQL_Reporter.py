...
def mail(recepients,data,subject,type):
    
    curr_date = date.today()
    msg = MIMEMultipart('alternative')
    sender = "damir@excitedigitalmedia.com"
    msg['Subject'] = subject #+ curr_date.strftime('%Y-%m-%d')
...
def main(file,type):
    ...
    try:
        ...
        try:
            yest_date = Config.get('REPORT CONFIG','DATE')
            if yest_date == "yesterday":
                subject = subject + " - " + (date.today() - timedelta(days=1)).strftime("%Y-%m-%d")
            if yest_date == "today":
                subject = subject + " - " + date.today().strftime("%Y-%m-%d")
        except:
            print("No Date")
        ...
        try:
            ...
            if 'success' in data:
                mail(recepients,data[1],subject,type)
            else:
